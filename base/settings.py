...
SECRET_KEY = config(
    "SECRET_KEY", cast=str, default="hello_worldfnjjnbfgbfgbjnkfgbnjkfnjgkbjkf894966"
)
...
DATABASES = {
    "default": {
        ...
        "OPTIONS": {
            "pool": {
                "min_size": config("POOL_MIN_SIZE", cast=int, default=3),
                "max_size": config(
                    "POOL_MAX_SIZE", cast=int, default=20
                ),  # for 5 workers --> 5*20=100 connections maximum
                "timeout": config("POOL_TIMEOUT", cast=int, default=30),
            }
        },
    }
}
...
)  # when manage.py collect-static save static files
...
DEFAULT_AUTO_FIELD = config(
    "DEFAULT_AUTO_FIELD", cast=str, default="django.db.models.BigAutoField"
)
...
if USE_SSL_CONFIG:
    ...
    SECURE_REFERRER_POLICY = (
        "strict-origin-when-cross-origin"  # control information  on sourse request
    )
...
if USE_CORS:
    ...
    CORS_ALLOWED_ORIGINS = config(
        "PRODUCTION_CORS_ALLOWED_ORIGINS", cast=Csv(), default=""
    )
...
if USE_LOG and DEBUG:
    ...
    LOGGING["handlers"]["console"] = {"class": "logging.StreamHandler", "LEVEL": "INFO"}

# jwt config
SIMPLE_JWT = {
    "REFRESH_TOKEN_LIFETIME": timedelta(
        days=config("REFRESH_TOKEN_LIFETIME", cast=int, default=30)
    ),
    ...
    "BLACKLIST_AFTER_ROTATION": config(
        "BLACKLIST_AFTER_ROTATION", cast=bool, default=True
    ),
...
USE_SPECTACULAR_EXTRAS_SETTINGS = config(
    "USE_SPECTACULAR_EXTRAS_SETTINGS", cast=bool, default=True
)
...
if USE_CLICKHOUSE_DB:
    ...
    CLICKHOUSE_DB_PASSWORD = config(
        "CLICKHOUSE_DB_PASSWORD", cast=str, default="new_password"
    )
    CLICKHOUSE_DB_DATABASE = config(
        "CLICKHOUSE_DB_DATABASE", cast=str, default="default"
    )
